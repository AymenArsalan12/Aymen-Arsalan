# Hi there, I'm  Aymen-Arsalan 

###About Me
- 👋 I am a diploma student with a strong interest in software development and modern technologies.
- 💻 Passionate about programming, web development, and building practical projects to enhance my technical skills.
- 📚 Continuously learning and improving my knowledge in coding, problem-solving, and application development.
-🚀 Dedicated to creating innovative solutions and growing as a future software engineer.
-🤝 Interested in collaboration, open-source contributions, and gaining real-world development experience.


### 🛠️ Skills
- 🌐 Beginner in HTML and web page development
- 🐍 Beginner in Python programming
- 💻 Completed a basic project using HTML
- 📚 Continuously learning and improving programming skills
- 🚀 Interested in software development and modern technologies


## Connect with ME 
Connect with me on 
   - aymenarsalan12@gmail.com
   - https://www.linkedin.com/in/aymen-arsalan-a876503b1
- 🤝 Open to connecting with developers, students, and tech enthusiasts.
- 📫 Feel free to connect with me for collaboration and learning opportunities.
- 🌐 Always interested in networking, learning, and working on new projects.
